/*
 * This module makes use of static variables for convenience,
 * but maybe does not conform too strictly to modular independence.
 * Maybe that could be an improvement to make for the future
*/
...
main(int argc, char * argv[])
{
    ...
    /* if copying one file to another, open output file first */
    ...
    {
        /* open output file */
        /* this is the case when we are copying files to a directory */
        if(!outfile_open)
        {
            /* create path with filename same as input file, since only directory provided */
            sprintf(outfile, "%s%s%s", OUTFILE, (OUTFILE[strlen(OUTFILE) - 1] == '/' ? "" : "/"), argv[i]);

            /* attempt to open the file */
            int rc = open_output_file(outfile);

            /* case file already exists, and user declined to overwrite it */
 ...
 * Description:
 *      Opens input file with proper checks
*/
static int
...
transfer_data()
{
    ...
    /* transfer 1kb of data at a time */
    while((bytes_read = fread(buffer, 1, BUF_SIZE, fd_in[fileno])) > 0)
    {
